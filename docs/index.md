# Payment Channels
Orecore includes classes necessary to handle payment channel smart contracts. The approach taken is networking-agnostic, so this library can be used at any layer.
...
```javascript
var Consumer = require('orecore-channel').Consumer;
...
Now that we have instantiated our object, we have two ways of funding the channel. The basic one is to send galactrum to an address that is provided by the Consumer instance (a private key is created for this purpose).

```javascript
console.info('Send galactrum to ' + consumer.fundingAddress.toString() ' to fund the channel');

consumer.processFunding([{...}, {...}, {...}]);
```

The objects that the consumer can understand are those returned by the Insight API: [https://github.com/GAB5TER/insight-api#unspent-outputs](https://github.com/GAB5TER/insight-api#unspent-outputs)
...
```javascript
var Provider = require('orecore-channel').Provider;
var paymentAddress = 'yf83ixbQdytYBpFkrCxGCdnUSGzumhyPjS';
...
sendToProvider(consumer.incrementPaymentBy(400 * uORE));
sendToProvider(consumer.incrementPaymentBy(4 * mORE));
```
...
assert(provider.currentAmount === 8 * mORE);
```
